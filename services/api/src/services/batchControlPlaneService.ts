...
export class BatchControlPlaneService {
  ...
  async createBatchFromProvider(params: {
    ...
    try {
      ...
      this.logger.info(
        { batchId, acquisitionProvider: params.providerId },
        "batch created",
      );
      return batch;
    } catch (error) {
  ...
  async parseBatch(batchId: string): Promise<BatchRecord> {
    ...
    try {
      ...
      this.logger.info({ batchId, workItems: result.workItems.length }, "batch parsed");
      return batch;
    } catch (error) {
      batch.status = "FAILED";
      batch.updatedAt = new Date().toISOString();
  ...
  async getKnownPatientArtifacts(batchId: string, patientId: string): Promise<{
    ...
    const artifactPaths = {
      ...
      oasisDiagnosisCompare: path.join(patientArtifactsDirectory, "oasis-diagnosis-compare.json"),
      oasisDiagnosisSnapshot: path.join(patientArtifactsDirectory, "oasis-diagnosis-snapshot.json"),
      oasisDiagnosisVerification: path.join(patientArtifactsDirectory, "oasis-diagnosis-verification.json"),
      oasisExecutionResult: path.join(patientArtifactsDirectory, "oasis-execution-result.json"),
      oasisInputActions: path.join(patientArtifactsDirectory, "oasis-input-actions.json"),
      oasisLockState: path.join(patientArtifactsDirectory, "oasis-lock-state.json"),
      oasisReadyDiagnosis: path.join(patientArtifactsDirectory, "oasis-ready-diagnosis.json"),
    };
    ...
    return {
      ...
      artifactContents: {
        ...
        oasisDiagnosisCompare: await this.repository.readJsonIfExists(artifactPaths.oasisDiagnosisCompare),
        oasisDiagnosisSnapshot: await this.repository.readJsonIfExists(artifactPaths.oasisDiagnosisSnapshot),
        oasisDiagnosisVerification: await this.repository.readJsonIfExists(artifactPaths.oasisDiagnosisVerification),
        oasisExecutionResult: await this.repository.readJsonIfExists(artifactPaths.oasisExecutionResult),
        oasisInputActions: await this.repository.readJsonIfExists(artifactPaths.oasisInputActions),
        oasisLockState: await this.repository.readJsonIfExists(artifactPaths.oasisLockState),
        oasisReadyDiagnosis: await this.repository.readJsonIfExists(artifactPaths.oasisReadyDiagnosis),
      },
    };
  }

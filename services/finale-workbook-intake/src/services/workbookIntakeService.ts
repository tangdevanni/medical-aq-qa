...
export async function intakeWorkbook(
  ...
  const manifest = createBatchManifest({
    ...
    workbookPath: params.workbookPath,
    outputDirectory,
    workItems: aggregation.workItems,
    parserExceptions,
  });
  ...
  await writeFile(workItemsPath, JSON.stringify(aggregation.workItems, null, 2), "utf8");
  await writeFile(normalizedPatientsPath, JSON.stringify(aggregation.workItems, null, 2), "utf8");
  await writeFile(parserExceptionsPath, JSON.stringify(parserExceptions, null, 2), "utf8");

  logger.info(
    {
      batchId,
      workItems: aggregation.workItems.length,

...
import type { RawExtractedTextSource } from "./documentTextAnalysis";
...
export type DocumentTextExportDocument = {
  ...
  rawExtractedTextSource: RawExtractedTextSource | null;
...
export async function writeDocumentTextFile(input: {
  ...
  const documents = input.extractedDocuments.map((document, documentIndex) => ({
    ...
    rawExtractedTextSource: document.metadata.rawExtractedTextSource ?? null,
    textSelectionReason: normalizeWhitespace(document.metadata.textSelectionReason) || null,
    domExtractionRejectedReasons: asStringList(document.metadata.domExtractionRejectedReasons),

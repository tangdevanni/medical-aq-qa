...
export class PlaywrightBatchQaWorker implements BatchPortalAutomationClient {
  ...
  async initialize(outputDir?: string): Promise<void> {
    ...
    const loginStepLogs = await loginPage.ensureLoggedIn({
      baseUrl: this.env.PORTAL_BASE_URL,
      username: this.env.PORTAL_USERNAME,
      password: this.env.PORTAL_PASSWORD,
    });
    this.dashboardUrl = this.resolveDashboardUrl({
      configuredDashboardUrl: this.env.PORTAL_DASHBOARD_URL,
      currentUrl: this.session.page.url(),
    });
    this.logger.info(
      {
        configuredDashboardUrl: this.env.PORTAL_DASHBOARD_URL ?? null,
    ...
    this.pendingInitializationStepLogs = [
      ...
      createAutomationStepLog({
        ...
        urlBefore: this.env.PORTAL_BASE_URL ?? null,
        ...
        evidence: [
          ...
          `configuredDashboardUrl=${this.env.PORTAL_DASHBOARD_URL ?? "unset"}`,
  ...
  async resolvePatient(workItem: PatientEpisodeWorkItem, evidenceDir?: string): Promise<{
    ...
    this.logger.info({ workItemId: workItem.id }, "matching patient");
  ...
  async discoverArtifacts(
    ...
    this.logger.info({ workItemId: workItem.id }, "discovering chart artifacts");
  ...
  private async bootstrapDashboardContextForPatientLookup(input: {
    ...
    const missingPortalBootstrapConfig = !this.env.PORTAL_BASE_URL && !dashboardUrl;
    ...
    stepLogs.push(
      createAutomationStepLog({
        ...
        evidence: [
          `PORTAL_BASE_URL=${this.env.PORTAL_BASE_URL ?? "unset"}`,
          `configuredDashboardUrl=${this.env.PORTAL_DASHBOARD_URL ?? "unset"}`,
    ...
    if (missingPortalBootstrapConfig) {
      stepLogs.push(
        createAutomationStepLog({
          ...
          urlBefore: currentUrl,
          urlAfter: currentUrl,
          missing: ["PORTAL_BASE_URL or PORTAL_DASHBOARD_URL"],
          evidence: [
  ...
  private async runFallbackDashboardReset(input: {
    ...
    if (dashboardReset.ready) {
      this.dashboardUrl = this.resolveDashboardUrl({
        configuredDashboardUrl: this.env.PORTAL_DASHBOARD_URL ?? this.dashboardUrl ?? undefined,
        currentUrl: this.session.page.url(),
      }) ?? input.dashboardUrl;
    }

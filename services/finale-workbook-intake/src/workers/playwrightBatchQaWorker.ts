...
async function captureVisitNoteRowReadOnly(input: {
  ...
  return withTimeout((async () => {
    ...
    if (download) {
      ...
      const [sourceHtml, sourceText] = await Promise.all([
...
export class PlaywrightBatchQaWorker implements BatchPortalAutomationClient {
  ...
  async debugFileUploadsDiscovery(
    ...
    return patientChartPage.debugFileUploadsDiscovery({
  ...
  async validateOasisAssessmentForReview(input: {
    ...
    return patientChartPage.validateOasisAssessmentForReview({
      chartUrl: this.currentPatientChartUrl ?? input.context.chartUrl,
      assessmentType: input.assessmentType,
      patientKey: input.workItem.id,
  ...
  async discoverVisitNotesForReview(input: {
    ...
    for (const row of capturePlan.rows) {
      if (row.captureStatus !== "not_attempted" || row.captureEligibility !== "active_monitoring") {
      ...
      {
        const restoreAfterCapture = await withTimeout(
          this.ensurePatientChartContextForOasis({
            context: input.context,
            workItem: input.workItem,
            evidenceDir: input.evidenceDir,
            reason: "restore_patient_chart_after_visit_note_capture",
          }),
          ...
          stepLogs: [createAutomationStepLog({
            ...
            message: "Timed out restoring patient chart context after Visit Note capture; continuing the patient run.",
            patientName: input.workItem.patientIdentity.displayName,
            evidence: [error instanceof Error ? error.message : String(error)],
            safeReadConfirmed: true,
          })],
        }));
        captureStepLogs.push(...restoreAfterCapture.stepLogs);
      }
    }

...
function extractPossibleIcd10Codes(inputText: string): string[] {
  const normalized = normalizeWhitespace(inputText).toUpperCase();
  if (!normalized) {
    return [];
  }

  const matches = normalized.match(/\b[A-TV-Z][0-9][0-9AB](?:\.[0-9A-TV-Z]{1,4})?\b/g) ?? [];
...
export class PatientChartPage {
  ...
  private async openFileUploadsAndAdmissionOrderFromSidebar(input: {
    ...
    if (admissionCandidates.length > 0) {
      ...
      admissionReasonSnippets = extractAdmissionReasonSnippets(admissionOrderTextExcerpt ?? "");
      admissionReasonPrimary = admissionReasonSnippets[0] ?? null;
      possibleIcd10Codes = extractPossibleIcd10Codes(admissionOrderTextExcerpt ?? "");
      const extractedTextLength = normalizeWhitespace(admissionOrderTextExcerpt).length;
      extractionSuccess = pdfResponseDetected || extractedTextLength >= 120 || possibleIcd10Codes.length > 0;
        ...
        JSON.stringify(
          {
            ...
            possibleIcd10Codes,
            textPreview: normalizeWhitespace(admissionOrderTextExcerpt).slice(0, 500) || null,
            generatedAt: new Date().toISOString(),
          },
          null,
          2,

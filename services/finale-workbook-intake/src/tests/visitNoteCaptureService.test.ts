...
describe("visit note controlled capture scaffold", () => {
  ...
  it("captures all eligible and finalized clinical notes by default when no cap is configured", () => {
    const discovery = buildVisitNotesDiscoveryArtifactForTest({
      rows: [
        ...
        { rawDocumentType: "Visit Note-PTA", statusRaw: "QA Completed", hasSafeOpenAction: true, rowText: "Visit Note-PTA QA Completed" },
      ],
    });
    const plan = planControlledVisitNoteCapture({ discovery });

    expect(plan.eligibleForCapture).toBe(3);
    expect(plan.boundedCaptureCount).toBe(3);
    expect(plan.rows.map((row) => row.captureStatus)).toEqual(["not_attempted", "not_attempted", "not_attempted"]);
    expect(plan.rows[2]?.lifecycleStatus).toBe("finalized_no_active_monitoring");
  });
  ...
  it("captures QA Complete clinical notes for final check without active monitoring", () => {
    const discovery = buildVisitNotesDiscoveryArtifactForTest({
      rows: [
  ...
  it("reruns final check when an active note becomes QA Complete", () => {

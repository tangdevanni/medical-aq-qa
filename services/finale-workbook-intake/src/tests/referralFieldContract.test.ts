...
describe("referral field contract", () => {
  it("generates deterministic field buckets from the contract", () => {
    ...
    expect(snapshot.fields.some((field) => field.key === "gg_self_care")).toBe(true);
    expect(snapshot.fields.some((field) => field.key === "gg_mobility")).toBe(true);
  });

  it("preserves printed-note OCR provenance when chart values come from printed-note extraction", () => {
    const snapshot = buildFieldMapSnapshot({
      chartSnapshotValues: createInitialChartSnapshotValues({
        workItem: {
          id: "patient-1",
        ...
        currentChartValues: {
          gg_self_care: "Supervision or touching assistance",
        },
        currentChartValueSource: "printed_note_ocr",
      }),
    });

    const ggSelfCare = snapshot.fields.find((field) => field.key === "gg_self_care");
    expect(ggSelfCare?.currentChartValue).toBe("Supervision or touching assistance");
    expect(ggSelfCare?.currentChartValueSource).toBe("printed_note_ocr");
    expect(ggSelfCare?.populatedInChart).toBe(true);
  });
});

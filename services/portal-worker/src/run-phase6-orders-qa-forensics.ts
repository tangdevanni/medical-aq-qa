...
function buildPhase6OrdersQaForensicsJob(
  env: ReturnType<typeof loadPortalWorkerEnv>,
): PortalJob {
  const jobId = `phase6-${randomUUID()}`;

  return {
    jobId,
    portal: PORTAL_NAMES.finaleHealth,
    portalUrl: env.portalBaseUrl,
    requestedBy: "local-phase6-orders-qa-forensics-runner",
    credentials: {
      username: env.portalUsername,
      password: env.portalPassword,
    },
    payload: {
      workflow: PORTAL_WORKFLOW_NAMES.ordersQaInteractionForensics,
      permissions: "read_only_navigation",
      source: "local-phase6-orders-qa-forensics-runner",
      sourceType: "manual",
      readOnly: true,
    },
  };
...
async function main(): Promise<void> {
  const logger = createLogger({ service: "portal-worker-phase6" });
  ...
  const job = buildPhase6OrdersQaForensicsJob(env);

  logger.info("Starting Phase 6 Orders QA interaction forensics run.", {
    jobId: job.jobId,
  ...
  logger.info("Completed Phase 6 Orders QA interaction forensics run.", {
    jobId: job.jobId,
    workflow: job.payload?.workflow,
    portal: job.portal,
    baseUrl: env.portalBaseUrl,
...
void main().catch((error: unknown) => {
  const logger = createLogger({ service: "portal-worker-phase6" });
  logger.error("Phase 6 Orders QA interaction forensics run failed.", {

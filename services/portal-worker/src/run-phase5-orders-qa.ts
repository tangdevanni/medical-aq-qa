...
function buildPhase5OrdersQaJob(env: ReturnType<typeof loadPortalWorkerEnv>): PortalJob {
  const jobId = `phase5-${randomUUID()}`;

  return {
    jobId,
    portal: PORTAL_NAMES.finaleHealth,
    portalUrl: env.portalBaseUrl,
    requestedBy: "local-phase5-orders-qa-runner",
    credentials: {
      username: env.portalUsername,
      password: env.portalPassword,
    },
    payload: {
      workflow: PORTAL_WORKFLOW_NAMES.ordersQaEntryDiscovery,
      permissions: "read_only_navigation",
      source: "local-phase5-orders-qa-runner",
      sourceType: "manual",
      readOnly: true,
    },
  };
...
async function main(): Promise<void> {
  const logger = createLogger({ service: "portal-worker-phase5" });
  ...
  const job = buildPhase5OrdersQaJob(env);

  logger.info("Starting Phase 5 Orders QA discovery run.", {
    jobId: job.jobId,
  ...
  logger.info("Completed Phase 5 Orders QA discovery run.", {
    jobId: job.jobId,
    workflow: job.payload?.workflow,
    portal: job.portal,
    baseUrl: env.portalBaseUrl,
...
void main().catch((error: unknown) => {
  const logger = createLogger({ service: "portal-worker-phase5" });
  logger.error("Phase 5 Orders QA discovery run failed.", {

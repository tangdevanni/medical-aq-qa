...
function normalizeDiagnosisCandidate(value: string): string {
  return normalizeDiagnosisPhrase(
    sanitizeFreeText(value)
      .replace(/^[A-Z0-9.]+\),\s*/g, " ")
      .replace(/\(\s*[A-Z1L][0-9][0-9AB](?:\.[0-9A-TV-Z]{1,4})?\s*\)/gi, " ")
...
function buildDiagnosisCodePair(input: {
  ...
  return {
    diagnosis,
    code: normalizeIcd10Code(input.code ?? null),
...
function extractDiagnosesFromCodePrefixedText(text: string): string[] {
  ...
  const matches = Array.from(
    normalized.matchAll(
      /\b(?:[A-Z1L][0-9][0-9AB](?:\.[0-9A-TV-Z]{1,4})?)\s+([A-Z][A-Za-z0-9 ,()'\/-]{3,}?)(?=\s+(?:\d{2}\/\d{2}\/\d{4}\b|Primary\b|Other\b|\b[A-Z1L][0-9][0-9AB](?:\.[0-9A-TV-Z]{1,4})?\b|$))/g,
    ),
  );
...
function extractDiagnosesFromParenthesizedList(text: string): string[] {
  ...
  const matches = Array.from(
    normalized.matchAll(
      /(?:^|[,;:]\s+)([A-Z][A-Za-z0-9 ,()'\/-]{3,}?)(?=\s*\((?:[A-Z1L][0-9][0-9AB](?:\.[0-9A-TV-Z]{1,4})?)\))/g,
    ),
  );
...
function extractStructuredDiagnosisEntries(value: unknown): Array<{
  ...
  return value.flatMap((entry) => {
    ...
    return [{
      diagnosis,
      code: normalizeIcd10Code(sanitizeFreeText((entry as PartialOasisDiagnosisEntry).code)) ?? null,
...
function normalizeDiagnosisCodePairs(
  ...
  for (const rawPair of value) {
    ...
    const code = codeRaw ? normalizeIcd10Code(codeRaw) : null;
    if (codeRaw && !code) {
...
function canonicalizeExtractionCandidate(input: {
  ...
  const explicitCodes = uniqueCaseInsensitive([
    ...toStringArray(input.candidate.icd10_codes_found_verbatim).map((code) => normalizeIcd10Code(code) ?? ""),
    ...toStringArray(input.fallback.icd10_codes_found_verbatim).map((code) => normalizeIcd10Code(code) ?? ""),
  ...
  const malformedCodes = uniqueCaseInsensitive(
    [
      ...
      .filter((code) => Boolean(code) && !Boolean(normalizeIcd10Code(code))),
  );
